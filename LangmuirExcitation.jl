...
for it in axes(psd.t_run, 1)
    v_profiles = v_dfdvmax[:, it]
    v_max[it] = v_profiles
    #E_profiles = E_ev.(v_profiles, c.me_ev)
    nmax = maximum(length.(v_profiles[:]))
    v_mat = stack(rpad_array.(v_profiles, nmax, NaN))'
    #E_mat = stack(rpad_array.(E_profiles, nmax, NaN))
    k_growth[it] = wp ./ sqrt.(v_mat.^2 .- 3/2*vth .^2)
    #w_growth = k_growth .* vmax
end
